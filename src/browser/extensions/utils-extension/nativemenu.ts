 ...
 */
export class NativeMenu implements IMainMenu {
  ...
  addMenu(menu: Menu, options: IMainMenu.IAddOptions = {}): Promise<void> {
    if (
      ArrayExt.findFirstIndex(this._items, (item, idx) => item.menu === menu) >
      -1
    ) {
      return;
    }
...
function dedupSeparatorsMenu(
  arr: INativeMenu.IMenuItemOptions[]
): INativeMenu.IMenuItemOptions[] {
