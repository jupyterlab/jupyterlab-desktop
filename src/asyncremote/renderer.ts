...
export interface IAsyncRemoteRender {
  createRemoteMethod: <T, U>(
...
class RenderRemote implements IAsyncRemoteRender {
  constructor() {
    ...
    ipcRenderer.on(
      Utils.EMIT_EVENT,
      (event: Electron.Event, data: Utils.IEventEmit<any>) => {
  ...
  runRemoteMethod<T, U>(method: AsyncRemote.IMethod<T, U>, arg: T): Promise<U> {
    ...
    return new Promise<U>((res, rej) => {
      ...
      let handler = (
        evt: Electron.Event,
        data: Utils.IMethodExecuteResponse<U>
      ) => {
...
export let asyncRemoteRenderer = asyncRemote;
